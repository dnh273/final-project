...
const fetchListNghienCuuKhoaHocAndUpdate = async () => {
  const oldestData = await NghienCuuKhoaHoc.find().sort({ createdAt: -1 }).limit(1);
  ...
  if (response.status === 200) {
    const ListIdNghienCuuKhoaHocDelete = ListNghienCuuKhoaHoc.filter(
      (item) =>
        !response.data.ListNghienCuuKhoaHoc.map((data: INghienCuuKhoaHoc) => data._id).includes(
          item._id.toString() 
        )
    ).map((item) => {
      _id: item.id;
    ...
    if (ListIdNghienCuuKhoaHocDelete.length > 0) {
      await NghienCuuKhoaHoc.deleteMany({ _id: { $in: ListIdNghienCuuKhoaHocDelete } });
    ...
    if (response.data.ListNghienCuuKhoaHoc.length > 0 && oldestData.length > 0) {
      const ListNghienCuuKhoaHocInsert = response.data.ListNghienCuuKhoaHoc.filter(
      ...
      const bulkOperations = ListNghienCuuKhoaHocUpdate.map((update: INghienCuuKhoaHoc) => {
        return {
          updateOne: {
